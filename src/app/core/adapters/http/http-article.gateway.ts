...
export class HttpArticleGateway extends ArticleGateway {
    ...
    override getArticleTypes(): Observable<Type[]> {
        return this.http.get<Type[]>(environment.prodURL + '/articles/articles/types')
    }

    override getArticlesByType(type_id: number): Observable<Article[]> {
        return this.http.get<FetchArticlesResponse>(environment.prodURL + `/articles/pro/${type_id}/1/10`).pipe(
            map(response => response.data)
        )
    }

    override getLastArticlesByType(): Observable<Article[][]> {
        return this.http.get<FetchArticlesResponse>(environment.prodURL + '/articles/pro/0/1/1').pipe(
            map(response => response.data)
        )
    }
